...
namespace GPUInstancer
{
    public static class GPUInstancerUtility
    {
        ...
        {
#if UNITY_EDITOR
            string pipeline = GPUInstancerConstants.gpuiSettings.isLWRP ? "LWRP" : "URP";
            ...
            {
                string packagePath = GPUInstancerConstants.gpuiSettings.isLWRP ? GPUInstancerConstants.FOLIAGE_SHADER_LWRP_PACKAGE_PATH : GPUInstancerConstants.FOLIAGE_SHADER_URP_PACKAGE_PATH;
            ...
            GPUInstancerTerrainSettings terrainSettings, int detailLayer)
        {
            for (int i = 0; i < detailPrototypes.Count; i++)
            {
                ...
                if (detailPrototype.usePrototypeMesh)
                {
                    ...
                    if (detailPrototypes[i].prefabObject.GetComponentsInChildren<MeshRenderer>()
                        .Any(r => r.sharedMaterial.shader.name == GPUInstancerConstants.SHADER_GPUI_FOLIAGE || r.sharedMaterial.shader.name == GPUInstancerConstants.SHADER_GPUI_FOLIAGE_URP
                               || r.sharedMaterial.shader.name == GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP))
                    {
                        for (int lod = 0; lod < runtimeData.instanceLODs.Count; lod++)
                        {
                            for (int r = 0; r < runtimeData.instanceLODs[lod].renderers.Count; r++)
                ...
                else
                {
                    ...
                    instanceMaterial = new Material(Shader.Find(GPUInstancerConstants.gpuiSettings.isURP ? GPUInstancerConstants.SHADER_GPUI_FOLIAGE_URP : 
                         GPUInstancerConstants.gpuiSettings.isLWRP ? GPUInstancerConstants.SHADER_GPUI_FOLIAGE_LWRP : GPUInstancerConstants.SHADER_GPUI_FOLIAGE));
